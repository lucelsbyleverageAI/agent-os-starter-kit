...
from agent_platform.agents.deep_research_agent.state import ResearchComplete, Summary
...
async def load_mcp_tools(
    ...
    # Step 1: Always fetch authentication using central service when MCP config is present
    mcp_auth_data = None
    if configurable.mcp_config:
        mcp_auth_data = await central_fetch_tokens(config)
    
    # Step 2: Validate configuration requirements (require auth data to proceed)
    config_valid = (
    ...
    # Step 3: Set up MCP server connection (parity with tools_agent)
    # Ensure trailing slash as some clients expect it
    server_url = configurable.mcp_config.url.rstrip("/") + "/mcp"

    # Configure authentication headers if auth data is available
    auth_headers = None
    if mcp_auth_data:
        ...
        if auth_type == "mcp_access_token":
            ...
            if access_token:
                headers["Authorization"] = f"Bearer {access_token}"
        # Legacy custom headers are no longer supported by the MCP server
        headers["Accept"] = "application/json, text/event-stream"
        auth_headers = headers

    # Step 4: Load tools from MCP server using low-level client for parity
    configured_tools: list[BaseTool] = []
    try:
        async with streamablehttp_client(server_url, headers=auth_headers) as streams:
            ...
            async with ClientSession(read_stream, write_stream) as session:
                ...
                while True:
                    ...
                    for mcp_tool in tool_list_page.tools:
                        ...
                        configured_tools.append(wrapped_tool)
...
async def get_all_tools(config: RunnableConfig):
    ...
    try:
        ...
        supabase_token = (
            ...
            config.get("configurable", {}).get("supabaseAccessToken")
        )
