# Supabase in the Agent Platform

This directory contains a curated, self-hosted Supabase stack, tailored for the Agent Platform. It provides all the core open-source Supabase services (Postgres, Auth, Storage, Realtime, REST, GraphQL, Kong API Gateway, etc.) as Docker containers, with configuration and orchestration optimised for local development, staging, and production.

## How We Use Supabase

- **Self-Contained Stack**: We copy only the essential Docker and configuration files from the official Supabase repo, versioned in `supabase/docker/` and `supabase/volumes/`. This allows us to customise and upgrade independently.
- **Environment-Specific Orchestration**: The root-level `docker-compose.local.dev.yml`, `docker-compose.staging.yml`, and `docker-compose.production.yml` files orchestrate the Supabase stack for each environment. They:
  - Start all Supabase services as containers
  - Use Kong as a unified API gateway (with custom config in `volumes/api/`)
  - Integrate with our application services (LangConnect, MCP, Web) and workflow tools (n8n, Windmill)
  - Use Caddy (via Coolify) for subdomain routing in staging/production
- **Database Migrations**: We use a custom migration system (`database/migrate.py` and `database/migrations/`) to manage our own schema, separate from Supabase's internal migrations. The `migration-init` service ensures all migrations are applied before app services start.
- **Configuration**: All environment variables and secrets are managed via `.env` files and Compose overrides. See the main repo README for a full list.
- **Upgrading**: To upgrade Supabase, update the images/tags in the Compose files and copy any new/changed config files from upstream into `volumes/` as needed.
