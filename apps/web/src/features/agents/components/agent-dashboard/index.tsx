...
export function AgentDashboard() {
  const { agents, loading: agentsLoading, discoveryData } = useAgentsContext();
  const graphNameById = useMemo(() => {
    const map: Record<string, string> = {};
    (discoveryData?.valid_graphs || []).forEach(g => { if (g?.name) map[g.graph_id] = g.name; });
    return map;
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          >
            <SelectTrigger className="h-9 min-w-[200px]">
              <SelectValue placeholder="All Agent Templates" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">All Agent Templates</SelectItem>
              {allGraphGroups.map((graph) => (
                ...
                >
                  {graphNameById[graph.graphId] || _.startCase(graph.graphId)}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
