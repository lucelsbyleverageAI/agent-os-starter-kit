...
export function GraphSelect({
  ...
  const graphNameById = Object.fromEntries((discoveryData?.valid_graphs || []).map(g => [g.graph_id, g.name]).filter(([_, v]) => !!v));
      ...
      <SelectContent>
        <SelectGroup>
          {graphs.map((graph) => (
            ...
            >
              {graphNameById[graph.graph_id] || _.startCase(graph.graph_id)}
            </SelectItem>
          ))}
        </SelectGroup>
      </SelectContent>
