...
>({ children }: { children: React.ReactNode }): React.ReactElement {
  ...
  const fetchThreads = React.useCallback(
    async (agentId: string, deploymentId: string) => {
      if (!session?.accessToken) {
        const message = { title: "No access token found", key: "auth:no-access-token" };
        notify.error(message.title, { key: message.key });
        return;
      }
      if (!agentInboxId) {
        const message = { title: "No agent inbox ID found", key: "thread:no-agent-inbox" };
        notify.error(message.title, { key: message.key });
        return;
      }
      ...
      try {
        ...
        const message = threadMessages.fetch.error();
    ...
    [offsetParam, limitParam, agentInboxId, inboxParam, session?.accessToken],
  );
  ...
  const ignoreThread = async (threadId: string) => {
    ...
    try {
      ...
      const message = threadMessages.ignore.error();
      notify.error(message.title, {
        description: message.description,
        key: message.key,
      });
