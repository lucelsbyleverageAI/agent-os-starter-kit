"use client";

import React, { use, useEffect, useState } from "react";
import {
  Breadcrumb,
  BreadcrumbItem,
  BreadcrumbLink,
  BreadcrumbList,
  BreadcrumbPage,
  BreadcrumbSeparator,
} from "@/components/ui/breadcrumb";
import { AppHeader } from "@/components/app-header";
import { PageHeader } from "@/components/ui/page-header";
import { ArrowLeft } from "lucide-react";
import { useKnowledgeContext } from "@/features/knowledge/providers/Knowledge";
import { DocumentPageContent } from "@/features/knowledge/components/document-page-content";
import { getCollectionName } from "@/features/knowledge/hooks/use-knowledge";
import { Button } from "@/components/ui/button";
import { useRouter } from "next/navigation";
import { Skeleton } from "@/components/ui/skeleton";
import { useAuthContext } from "@/providers/Auth";

interface DocumentPageProps {
  params: Promise<{
    collectionId: string;
    documentId: string;
  }>;
}

/**
 * Dedicated page for viewing a document in a collection.
 * Accessible at /knowledge/[collectionId]/document/[documentId]
 */
export default function DocumentPage({ params }: DocumentPageProps): React.ReactNode {
  const { collectionId, documentId } = use(params);
  const router = useRouter();
  const { session } = useAuthContext();
  const { collections, initialSearchExecuted } = useKnowledgeContext();
  const [collection, setCollection] = useState<any>(null);
  const [documentTitle, setDocumentTitle] = useState<string>("Loading...");
  const [documentDescription, setDocumentDescription] = useState<string>("");
  const [loading, setLoading] = useState(true);

  // Find the collection from context
  useEffect(() => {
    if (initialSearchExecuted) {
      const found = collections.find((c) => c.uuid === collectionId);
      setCollection(found || null);
      setLoading(false);
    }
  }, [collectionId, collections, initialSearchExecuted]);

  // Fetch document title for breadcrumbs
  useEffect(() => {
    if (!session?.accessToken || !collectionId || !documentId) return;

    const fetchDocumentMeta = async () => {
      try {
        const response = await fetch(
          `/api/langconnect/collections/${collectionId}/documents/${documentId}`,
          {
            headers: {
              Authorization: `Bearer ${session.accessToken}`,
            },
          }
        );

        if (response.ok) {
          const doc = await response.json();
          setDocumentTitle(doc.title || "Untitled Document");
          setDocumentDescription(doc.description || "");
        }
      } catch (err) {
        console.error("Error fetching document metadata:", err);
      }
    };

    fetchDocumentMeta();
  }, [collectionId, documentId, session?.accessToken]);

  // Loading state
  if (loading || !initialSearchExecuted) {
    return (
      <React.Suspense fallback={<div>Loading...</div>}>
        <AppHeader>
          <Breadcrumb>
            <BreadcrumbList>
              <BreadcrumbItem>
                <BreadcrumbLink href="/knowledge">Knowledge</BreadcrumbLink>
              </BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Loading...</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </AppHeader>

        <div className="container mx-auto px-4 md:px-8 lg:px-12 py-6">
          <div className="space-y-4">
            <Skeleton className="h-12 w-64" />
            <Skeleton className="h-4 w-96" />
            <Skeleton className="h-[600px] w-full" />
          </div>
        </div>
      </React.Suspense>
    );
  }

  // Collection not found
  if (!collection) {
    return (
      <React.Suspense fallback={<div>Loading...</div>}>
        <AppHeader>
          <Breadcrumb>
            <BreadcrumbList>
              <BreadcrumbItem>
                <BreadcrumbLink href="/knowledge">Knowledge</BreadcrumbLink>
              </BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Not Found</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </AppHeader>

        <div className="container mx-auto px-4 md:px-8 lg:px-12 py-6">
          <div className="flex flex-col items-center justify-center gap-4 py-12">
            <h2 className="text-2xl font-semibold">Collection Not Found</h2>
            <p className="text-muted-foreground">
              The collection you're looking for doesn't exist or you don't have
              access to it.
            </p>
            <Button onClick={() => router.push("/knowledge")} variant="outline">
              <ArrowLeft className="mr-2 h-4 w-4" />
              Back to Knowledge
            </Button>
          </div>
        </div>
      </React.Suspense>
    );
  }

  const collectionName = getCollectionName(collection.name);

  return (
    <React.Suspense fallback={<div>Loading...</div>}>
      <AppHeader>
        <Breadcrumb>
          <BreadcrumbList>
            <BreadcrumbItem>
              <BreadcrumbLink href="/knowledge">Knowledge</BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbLink href={`/knowledge/${collectionId}`}>
                {collectionName}
              </BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbPage>{documentTitle}</BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
      </AppHeader>

      <div className="container mx-auto px-4 md:px-8 lg:px-12 py-6">
        <div className="mb-6">
          <Button
            onClick={() => router.push(`/knowledge/${collectionId}`)}
            variant="ghost"
            size="sm"
            className="mb-4"
          >
            <ArrowLeft className="mr-2 h-4 w-4" />
            Back to {collectionName}
          </Button>

          <PageHeader
            title={documentTitle}
            description={documentDescription || "View and manage this document"}
          />
        </div>

        <div className="mt-6">
          <DocumentPageContent
            documentId={documentId}
            collectionId={collectionId}
            onDocumentDeleted={() => {
              // Navigate back to collection after deletion with refresh flag
              router.push(`/knowledge/${collectionId}?refresh=${Date.now()}`);
            }}
          />
        </div>
      </div>
    </React.Suspense>
  );
}
