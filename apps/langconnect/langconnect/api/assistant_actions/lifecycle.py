        ...
        if deleted_from_langgraph:
            try:
                async with get_db_connection() as conn:
                    # Use transaction to ensure atomicity: if any step fails, all changes rollback
                    async with conn.transaction():
                        # Step 1: Manually deprecate threads before deletion
                        # This is required because migration 007 removed the FK constraint,
                        # so we manually deprecate threads before deletion as a defensive measure
